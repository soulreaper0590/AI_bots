...
def Algorithm(adjacency_list,start_node,goal_node,n_node):#you have to add the assigned algorith here
	path = []
	path2 = [None for y in range(n_node + 1)]
	path3 = [[None for y in range(n_node)] for x in range(n_node)]
	costs = [None for y in range(n_node)]
	inserted = [y  for y in range(n_node)]
	connection = [[0 for y in range(3)] for y in range(n_node**2)]
	start_node += -1
	for y in range(n_node):
		costs[y] = distance(start_node,y)
	for y in range(n_node):
		path3[inserted[y]][y] = start_node
		path3[inserted[y]][start_node] = y
	#print(path3)
	counter = 0
	for y in range(n_node):
		for k in range(n_node):
				#print(counter)
				connection[int(counter)] [0]= y
				connection[int(counter)][1] = k
				connection[int(counter)][2] = distance(start_node,y) + distance(k,start_node) - distance(y,k)
				counter = counter + 1
				#print(connection[int(counter)])
	connection.sort(key = takethrid, reverse = True) 
	#print(connection)
				# #print(y,k,start_node)
				# if(inserted[y] == inserted[k] or y == start_node or k == start_node):
				# 	continue
				# else:
				# 	if(maxi < distance(start_node,y) + distance(k,start_node) - distance(y,k) and path3[inserted[y]][start_node] == y and path3[inserted[k]][k] == start_node):
				# 		mini = distance(start_node,y) + distance(k,start_node) - distance(y,k)
				# 		#print("hoa caracola")
				# 		arg2 = y
				# 		arg1 = k 
	for z in range(n_node - 2):
		for p in range(n_node**2):
			y = connection[p][0]
			k = connection[p][1] 
			if(path3[inserted[y]][start_node] == y and path3[inserted[k]][k] == start_node and inserted[y] != inserted[k] and y != start_node and k != start_node):
				arg2 = y
				arg1 = k
				break
		i = arg2
		i2 = arg1
		# print(inserted)
		# print(arg1,arg2,start_node)
		# print(path3)
		while(1):
			#print(i2,inserted[i2],path3[inserted[i2]][i2],i)
			path3[int(inserted[i2])][int(i2)] =  int(i)
			p = int(path3[int(inserted[i])][int(i)])
			inserted[i] = int(inserted[i2])
			i2 = i
			i = p
			if(i == start_node):
				path3[inserted[i2]][i2]  = start_node
				break
		inserted[start_node] = inserted[i2]
	path = path3[inserted[y]]
	#print(path)
	path2[0] = start_node + 1
	counter = 2
	i = path[start_node]
	path2[1] = i + 1
	while(i!= start_node):
		path2[counter] = path[i]+1
		counter += 1
		i = path[i]
	#print(path2)
	return path2
...
def distance(i,j):
	...
	p1 = dict_index_coord[i + 1]
	p2 = dict_index_coord[j  + 1]
	x1 = p1[0]
	x2 = p2[0]
	y1 = p1[1]
	y2 = p2[1]
	var = abs(x1 - x2) + abs(y1 - y1)
